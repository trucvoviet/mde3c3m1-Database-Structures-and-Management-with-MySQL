...
SELECT EmployeeID, EmployeeName FROM employees WHERE EmployeeID = ANY(
...
SELECT DISTINCT EmployeeID, HandlingCost
FROM employee_orders 
WHERE HandlingCost > ALL (
SELECT ROUND(OrderTotal * 0.2) AS twentyPercent FROM orders
GROUP BY OrderTotal HAVING ROUND(twentyPercent) > 100
);
